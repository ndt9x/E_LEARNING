...
import com.example.database.Test;
...
public class test_lesson extends Activity implements OnClickListener {
	...
	private ArrayList<TestAnswer> arrList;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		database = new MySQLiteOpenHelper(test_lesson.this);
		arr = database.getTestLesson(bundle.getString("TestLesson"));
		arrList = new ArrayList<TestAnswer>();
		for (int i = 0; i < arr.size(); i++) {
			TestAnswer testAns = new TestAnswer(arr.get(i).getAnswerTrue(), "",
					arr.get(i).getId());
			arrList.add(testAns);
		}
	...
	private void showDialogResult(){
		final Dialog dialog = new Dialog(test_lesson.this);
